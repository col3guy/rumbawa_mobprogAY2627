...
- **Lab 6**: In Cloud Firestore, each registered user is stored in the `users` collection using their Firebase UID as the document ID. When two users initiate a chat, a chat document is created in the `chats` collection using their Firebase UIDs, while the `messages` subcollection stores their messages and details. If a user chats with themselves, the same Firebase UID will be used as both the sender and receiver, creating a self-chat.